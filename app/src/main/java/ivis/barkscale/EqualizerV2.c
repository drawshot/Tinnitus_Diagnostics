...
void EqualizerFunc_L(unsigned int BAND, unsigned int GAIN)
{
    //    printf("BAND : %d\n\r",BAND);
    for (int k = 0; k <= 11; k++)
    {
...
void EqualizerFunc_R(unsigned int BAND, unsigned int GAIN)
{
    //    printf("BAND : %d\n\r",BAND);
    for (int k = 0; k <= 11; k++)
    {
...
void EqualizerFunc(unsigned int BAND, unsigned int GAIN)
{
    //    printf("BAND : %d\n\r",BAND);
    for (int k = 0; k <= 11; k++)
    {
...
void setValue( uint8_t *receiveValue ){
    for (int i = 0 ; i < 24 ; i++){
        ...
        if (i<12) {
            EqualizerFunc_L(band[i], gain[gainIndex]);
        } else {
            EqualizerFunc_R(band[i], gain[gainIndex]);
        }
    }
    printf("end\n\r");
}
